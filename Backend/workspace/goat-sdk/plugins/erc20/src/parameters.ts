...
export class GetTokenBalanceParameters extends createToolParameters(
  z.object({
    wallet: z.string().describe('The wallet address to check the balance for'),
    tokenAddress: z.string().describe('The token contract address'),
    tokenDecimals: z.number().describe('The number of decimals for the token'),
    name: z
      .string()
      .describe('The token name/symbol for price lookup (e.g., "usdc")'),
  }),
) {}
...
export class GetTokenTotalSupplyParameters extends createToolParameters(
  z.object({
    tokenAddress: z
      .string()
      .describe('The address of the token to get the total supply of'),
  }),
) {}
...
export class ConvertToBaseUnitParameters extends createToolParameters(
  z.object({
    amount: z
      .number()
      .describe(
        'The amount of tokens to convert from decimal units to base units',
      ),
...
export class ConvertFromBaseUnitParameters extends createToolParameters(
  z.object({
    amount: z
      .number()
      .describe(
        'The amount of tokens to convert from base units to decimal units',
      ),
    decimals: z.number().describe('The number of decimals of the token'),
  }),
) {}
